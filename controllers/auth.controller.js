...
let signupController = async function (req, res, next) {
  try {
    ...
    if (!allowedRoles.includes(role)) {
      return next(
        new Apierror("Invalid role. Allowed roles are: user, admin", 400),
      );
    }
    ...
    res
      .status(201)
      .json({ message: "OTP sent successfully. Please verify your email." });
  } catch (error) {
    next(error);
  }
};
...
let forgetPasswordController = async function (req, res, next) {
  try {
    ...
    const resetLink = `http://localhost:4200/create-new-password/${resetToken}`;

    let resetPasswordEmail = await sendResetPasswordEmail(
      user.email,
      resetLink,
    );
...
let resetNewPasswordController = async function (req, res, next) {
  try {
    ...
    if (!newPassword || !ConfirmedNewPassword) {
      return next(
        new Apierror(
          "New Password and Confirmed New Password is required",
          400,
        ),
      );
    }
