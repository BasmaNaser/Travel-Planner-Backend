...
let verifyOtpController = async function(req,res,next){
    try {
        ...
        await OTP.deleteMany({email})
        res.status(200).json({message:'Account Created Successfuly',data:{
        id: user._id,
        email: user.email,
...
let resetPasswordController = async function (req, res, next) {
    try {
        ...
        res.status(200).json({message:'Reset token is valid',data:{
            id:user.id,
            email:user.email,
            role:user.role
        }})
...
module.exports={signupController,verifyOtpController,resendOtpController,loginController,refreshTokenController,forgetPasswordController,resetPasswordController,resetNewPasswordController}